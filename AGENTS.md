...
`check_cve(cve, all_files=False) -> dict | bool` (`cvehound/__init__.py:170`) is the only
...
live in `cvehound/__main__.py:371-405`.
...
`CVEHOUND_METADATA` env var (`cvehound/util.py:112`) or `--metadata`, and regenerated by
