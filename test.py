...
#def parse_cmd(obj_config, context, cmd):
#    res = parse_obj.match(cmd)
#    if res:
#        config = None
...
#        for obj in objs:
#            assert obj not in obj_config, "already " + obj
#            if config:
...
#                cmd += line
#            cmd = cmd.strip()
#            obj_config = parse_cmd(obj_config, context, cmd)
#            cmd = ''
#    return obj_config
#
...
#        obj_config = merge_obj_configs(obj_config, parse_makefile(join(tree['path'], makefile)))
...
#        elif obj.endswith('/'):
#            dir = obj[:-1]
#            assert '/' not in dir, "not nested dir"
#            path = tree['dirs'][dir]['path']
#            for file in tree['dirs'][dir]['cfiles']:
#                file = join(path, file)
#                if not file_config[file]['enabled']:
#                    file_config[file]['enabled'].extend(obj_config[obj]['enabled'])
#                file_config[file]['disabled'].extend(obj_config[obj]['disabled'])
#        else:
#            assert False, 'Unknown obj ' + obj
#    for file in configs:
#        file_config[join(tree['path'], file)] = configs[file]
#    iter += 1
#    if iter > 100:
#        pprint(file_config)
#        exit(0)

mk = Makefile('tests/linux')
mk.scan()
