...
class GitRepo:
    ...
    def log_commits(self, a: str, b: str, paths: Iterable[str]) -> list[str]:
        ...
        git's default history simplification, which is the chain of contents b
        actually passed through: for a merge that took the path from one side,
        that side's commit, not the merge. Asking for --ancestry-path here
        would turn the simplification off and list every merge whose diff
        touches the path -- 848 commits where the file has 13 contents.
        """
        out = self.run(
            'log',
            '--format=%H',
