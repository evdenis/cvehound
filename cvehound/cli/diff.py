...
# Stands in for a commit id in the evidence tables when the change is a patch.
PATCH_ID = '<patch>'
...
def main(args: list[str], prog: str = 'cvehound diff') -> int:
    ...
    with tempfile.TemporaryDirectory(prefix='cvehound-diff-', ignore_cleanup_errors=True) as root:
        ...
        if patch is not None:
            ...
            name = 'stdin' if args_cfg['patch'] == '-' else os.path.basename(args_cfg['patch'])
            label_to = f'{args_cfg["base"]} + {name}'
            tree_to = materialize_tree(
                materializer, repo, label_to, repo.patch_tree(base, patch), kind='tree'
            )
            # Not a sha, and spelled so it cannot be read as one: the evidence
            # tables are commit ids everywhere else.
            messages = iter([(PATCH_ID, patch.decode(errors='replace'))])
        else:
