...
def main(args: list[str], prog: str = 'cvehound bisect') -> int:
    ...
    with tempfile.TemporaryDirectory(prefix='cvehound-bisect-', ignore_cleanup_errors=True) as root:
        ...
        # Distinct contents, not len(steps): collapse_steps() merges *consecutive*
        # equal signatures, and a content that recurs -- a change and its revert --
        # opens a second step for the same blobs.
        contents = len({sig for _, sig in commits})
        ...
        print(
            ...
            f' between {rev_from} and {rev_to} ({contents} distinct contents)'
        )
