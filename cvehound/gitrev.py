...
class RevTree:
    ...
    `kind` is what `sha` names: a commit for a revision, a tree for the result
    of applying a patch, which has no commit to point a reader at.
    """
    ...
    def report(self) -> dict[str, Any]:
        return {'rev': self.rev, self.kind: self.sha, 'describe': self.describe}
...
def materialize_tree(
    ...
    out on a non-kernel. `sha` may be a commit or a bare tree (a --patch
    result); describe only means something for the former."""
    if paths is None:
    ...
    describe = repo.describe(sha) if kind == 'commit' else None
    return RevTree(label, sha, describe, materializer.materialize(sig), kind)
